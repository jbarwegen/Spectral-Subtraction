...
class SpectraSubtractionApp(qw.QMainWindow):
    ...
    def __init__(self):
        ...
        # Wire up required UI
        # This section connects each button to a function
    ...
    ####################
    # ACTIONS
    # Sets the directory where you want the spectra's to be saved
    def choose_save_location(self) -> None:
        """
        Opens a file dialog to select the directory where spectra images will be saved.

        Updates the 'save_path' attribute and the text of the save location line edit.
        """
    ...
    def load_excel_file(self) -> None:
        ...
        Retrieves the number of rows to skip from the UI, parses in the Excel data and stores the data as well as populates the UI menus with the Excel sheet names
        ...
        try:
            ...
            self.mainSpectraBox.clear()
            self.mainSpectraBox.addItems(names)
            self.subtractBox.clear()
            self.subtractBox.addItems(names)
            self.graphsWidget.clear()
            self.graphsWidget.addItems(names)
            self.spectraABox.clear()
            self.spectraABox.addItems(names)
            self.spectraBBox.clear()
            self.spectraBBox.addItems(names)
    ...
    def load_data(
        ...
        for name, df in raw.items():
            ...
            if missing:
                raise ValueError(
                    f"Sheet '{name}' is missing columns: {sorted(missing)}"
                )
            ...
            keep = keep.dropna(subset=["m/z", "Relative", "Resolution"]).reset_index(
                drop=True
            )
            filtered[name] = keep
    ...
    def plot_dual_spectrum(
        self, df_up: pd.DataFrame, df_down: pd.DataFrame, title: str, n_peaks: int = 10
    ) -> None:
        top_up = df_up.nlargest(n_peaks, "Relative") if not df_up.empty else df_up
        top_down = (
            df_down.nlargest(n_peaks, "Relative") if not df_down.empty else df_down
        )
        ...
        xmax = max(
            350,
            float(df_up["m/z"].max()) if not df_up.empty else 0,
    ...
    @staticmethod
    def compare_dfs(
        df1: pd.DataFrame, df2: pd.DataFrame, ppm_tol: float = 3.0
    ) -> pd.DataFrame:
        if df1.empty:
            return df1.copy()
        if df2.empty:
