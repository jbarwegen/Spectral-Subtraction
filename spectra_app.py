...
class SpectraApp(TkinterDnD.Tk):
    def __init__(self):
        ...
        self.status_label.grid(row=11, column=0, columnspan=2, sticky="we", padx=5, pady=5)
    ...
    def _handle_drop(self, event):
        ...
        path = event.data.strip("{}")  # Clean Windows/Mac style path
        if path.lower().endswith(('.xls', '.xlsx')):
            try:
                skip = int(self.skip_spin.get())
                self.sheet_names, self.data_dict = self.load_data(skip, path)
                self._populate_widgets()
                messagebox.showinfo("File Loaded", f"Loaded {len(self.sheet_names)} sheets from:\n{path}")
    ...
    def _populate_widgets(self):
        ...
        self.dual_listbox.delete(0, tk.END)
        for name in self.sheet_names:
            self.b_listbox.insert(tk.END, name)
            self.dual_listbox.insert(tk.END, name)

    def add_tooltip(self, widget, text):
        ToolTip(widget, text)

    def _create_widgets(self):
        ttk.Label(self, text="Rows to skip:").grid(row=0, column=0, padx=5, pady=5, sticky="w")
        ...
        self.add_tooltip(self.skip_spin, "Number of rows to skip at the top of each sheet (rows before headers)")

        load_btn = ttk.Button(self, text="Load File…", command=self.load_file)
        load_btn.grid(row=1, column=0, columnspan=2, pady=5)
        self.add_tooltip(load_btn, "Load an Excel file with spectra on separate sheets")
        ...
        self.add_tooltip(self.peak_spin, "Number of peaks to annotate in each spectrum plot")

        self.normalize_var = tk.BooleanVar(self)
        norm_cb = ttk.Checkbutton(self, text="Normalize", variable=self.normalize_var)
        ...
        save_cb = ttk.Checkbutton(self, text="Save Figures", variable=self.save_var)
        ...
        dual_cb = ttk.Checkbutton(self, text="Plot Dual Spectrum", variable=self.dual_plot_var)
        dual_cb.grid(row=10, column=0, columnspan=2, pady=5)
        self.add_tooltip(dual_cb, "Show a dual plot of positive and negative peaks after subtraction")
    ...
    def load_data(self, skip_rows, path):
        ...
        for name, df in raw.items():
            ...
            if not required.issubset(df.columns):
                raise ValueError(f"Sheet '{name}' is missing required columns: {required - set(df.columns)}")
    ...
    def run(self):
        ...
        if not selected_indices:
            messagebox.showwarning("No spectra selected", "Please select spectra to subtract from A.")
            return
        ...
        # Subtraction logic
        title = name_a + " Subtracted " + " Subtracted ".join(self.sheet_names[i] for i in selected_indices)
        df_down = self.data_dict[name_a].copy()
        for idx in selected_indices:
            name_d = self.sheet_names[idx]
            df_down = self.compare_dfs(df_down, self.data_dict[name_d])

        df_up = self.data_dict[name_a].copy()

        df_up = self.compare_dfs(self.data_dict[name_b], df_up)
    ...
    def plot_spectrum(self, df, N, names):
        ...
        if self.save_var.get():
            fig.savefig(names[0].replace(' ', '_') + '.svg')
            plt.close(fig)
        else:
            plt.show()
            plt.close(fig)
    ...
    def plot_dual_spectrum(self, df_up, df_down, N, title):
        ...
        for _, r in top_down.iterrows():
            ax.annotate(f"{r['m/z']:.4f}", xy=(r['m/z'], -r['Relative']), xytext=(0, -5),
                        textcoords='offset points', ha='center', va='top', rotation=45, fontsize=8, color='red')
        plt.tight_layout()
        if self.save_var.get():
